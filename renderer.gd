...
var player_vel: Vector2:
    get:
        return game.player_vel
var player_grounded: bool:
    get:
        return game.player_grounded
var camera_x: float:
    get:
        return game.camera_x
var shake_offset: Vector2:
    get:
        return game.shake_offset
var energy: float:
    get:
        return game.energy
var max_energy: float:
    get:
        return game.max_energy
var elapsed: float:
    get:
        return game.elapsed
var title_t: float:
...
var finish_t: float:
    get:
        return game.finish_t
var runner_anim: float:
    get:
        return game.runner_anim
var message: String:
    get:
        return game.message
var message_t: float:
    get:
        return game.message_t
var deaths: int:
    get:
        return game.deaths
var god_drawing: bool:
    get:
        return game.god_drawing
var god_draw_start: Vector2:
    get:
        return game.god_draw_start
var god_draw_end: Vector2:
    get:
        return game.god_draw_end
var solids: Array:
    get:
        return game.solids
var spikes: Array:
...
func _draw_runner() -> void:
    ...
    _paint_ellipse(p+Vector2(0,31),Vector2(26,8),Color(0,0,0,.28))
...
func _paint_ellipse(center: Vector2, radius: Vector2, color: Color) -> void:
    var pts = PackedVector2Array()
    for i in range(32):
        var a = TAU*i/32.0
...
func _draw_hud() -> void:
    _paint_panel(Rect2(34,28,360,58),Color(0.02,0.05,0.10,.78),Color(0.35,0.8,0.9,.22),22)
    ...
    var progress: float = clampf(player_pos.x / GOAL_X, 0.0, 1.0)
    _paint_panel(Rect2(860,28,386,58),Color(0.02,0.05,0.10,.72),Color(1,1,1,.08),22)
    ...
    if message_t > 0.0:
        var a: float = minf(1.0, message_t * 2.5)
        _paint_panel(Rect2(320,104,640,54),Color(0.02,0.045,0.085,.78*a),Color(0.45,0.95,1,.18*a),18)
        draw_string(ThemeDB.fallback_font,Vector2(345,139),message,HORIZONTAL_ALIGNMENT_CENTER,590,18,Color(0.88,0.98,1,a))

func _paint_panel(rect: Rect2, fill: Color, border: Color, radius: float) -> void:
...
func _draw_finish() -> void:
    var a: float = clampf(finish_t * 2.0, 0.0, 1.0)
